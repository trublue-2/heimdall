...
- **2026-07-08 (Prod-Diagnose: Brownout + Heap — beide unauffällig)** — Zwei zu Beginn des Umbaus unabhängig aufgekommene Verdachtsmomente (Brownout / Heap) gegen die echten Prod-Logs (`DeviceLog`, Box **LMB2**, FW 0.2.10) geprüft. **Brownout: keine** — Reset-Grund-Verteilung nur `DEEPSLEEP` (normal) + `SW` (OTA/Neustart), **0× `reset=BROWNOUT`/`PANIC`/WDT**. Der Banner-Zähler `unexp=31` ist ein Lifetime-NVS-Wert (Brownout **oder** Power-on gemeinsam) und historisch von Bench-/USB-Power-ons dominiert, kein Feld-Brownout. **Heap: kein Druck** — `min`-Watermark direkt nach dem doppelten TLS-Handshake (Sync + MQTT) bei ~135 kB frei (kleinster gemessener 138 632 B) von ~290 kB DRAM; MQTT 25 verbunden / 0 fehlgeschlagen → die ISRG-X1-only-Mitigation trägt. **Nebenbefund:** Watchdog (0.2.10) ist per OTA im Feld und stabil — keine WDT-/Reboot-Loop-Spuren (nur reguläre SW-Resets), er false-trippt nicht. Einschränkung: nur LMB2 hat `logToServer` aktiv, Log-Fenster praktisch nur der 07-07-Betriebstag — Snapshot einer Box/Session, keine Fleet-Historie.
- **2026-07-08 (Fix: MQTT-Auth-429 → `state=5`)** — Der Broker ruft `/api/mqtt/*` direkt aufs Docker-Netz (kein `X-Forwarded-For`) → `clientIp="unknown"` → der GANZE Fleet teilte sich EINEN Rate-Limit-Bucket (`BOX_LIMIT` ~8/min). Ein Reconnect-Burst (jeder Deploy trennt alle Boxen → gleichzeitiger Neuaufbau) oder Retry-Storm überschritt das → `429` → go-auth cachte das als deny → Boxen bekamen `MQTT-Connect (state=5, unauthorized)`. Fix (`proxy.ts`): interne Broker-Requests (`/api/mqtt/*`, `ip="unknown"`) vom Rate-Limit ausnehmen; externe `/api/mqtt/*` (via Traefik, echte IP) + `/api/box/*` bleiben gedrosselt. Broker-Log zeigte `error code: 429` im Minutentakt.
- **2026-07-08 (FW 0.2.18 — Akku live im Wachfenster + Server-Härtung)** — Solange die Box wach ist (= „Live"), resynct sie jetzt auch auf **Akku** periodisch (`DEBUG_RESYNC_MS`, bisher nur am USB) → Akku/RSSI im Dashboard live statt eingefroren vom letzten Sync. Verlängert das 2-min-Fenster nicht (schläft danach normal). **Server (0.2.17-Deploy):** `/open` sendet „open" nur noch, wenn `device.locked` (kein Motor gegen den Anschlag auch bei alter FW); Failsafe-Selbstöffnungen (Low-Batt/Offline/Deadline) werden als **`FAILSAFE_OPEN`** protokolliert (statt generisch „Geöffnet") und leeren die Heimdall-Sperre.
- **2026-07-08 (FW 0.2.17 — Failsafe-Oszillation + Stepper-gegen-Anschlag gefixt)** — **Bug A:** Akku ≤15 % → Low-Batt-Failsafe öffnet, aber der Sync wollte per `simpleLock` sofort wieder zu → Oszillation (Motorzyklen, Dauer-wach, Drain). `shouldClose = !Failsafe::shouldOpen(gBox, gPolicy)` (statt `!isPolicyExpired`) → ein Failsafe-Öffnen gewinnt, Box bleibt offen. **Bug B (Motor-Killer):** `OPENING` fuhr `Stepper::unlock()` bedingungslos → ein „open" auf eine schon offene Box (open-loop, kein Endlagensensor) trieb den Riegel gegen den mechanischen Anschlag (Stall/Motorschaden). Jetzt nur noch fahren, wenn `gBox.locked`; `lockBox()` analog idempotent; `reopen` bleibt bewusstes Re-Fahren. **Server:** ein Failsafe-Öffnen (`low_battery`/`offline_timeout`) leert die Heimdall-Policy → Kachel zeigt „OFFEN" statt dauerhaft „wird geschlossen".
- **2026-07-08 (Notöffnungs-Kontingent)** — Vorzeitiges „Trotzdem öffnen" (ohne Passwort) ist jetzt nur noch **X-mal** nutzbar: `Device.emergencyOpensLeft` (Default 3, Migration `add_emergency_quota`), dekrementiert je Early-Open in der Open-Route; **bei 0 blockiert** (409 „nur Keyholderin/Failsafes"). Kachel zeigt „X Notöffnungen übrig" (und im Modal „danach noch Y"); bei 0 statt Öffnen-Button ein Hinweis. Keyholder füllt das Kontingent auf der Detailseite auf (`DeviceSettingsForm` → PATCH `…/[deviceId]`). **Safety:** begrenzt nur das digitale vorzeitige Öffnen — Low-Batt/Offline/Deadline-Failsafes und der physische Break-Glass-Ausgang bleiben unberührt (`Safety > Security > Function`).
- **2026-07-08 (Dashboard-Kacheln + WLAN-Anzeige aufgeräumt)** — **Kachel:** kein Detail-Link mehr vom Dashboard (Prop `linkToDetail`, Default aus; Detailseite nur noch über „Geräte"). **Ein** Status-Indikator statt widersprüchlichem online+live: MQTT-verbunden → „Live", sonst „letzter Sync: vor X" (Dashboard-Kachel bekommt dafür `mqttLive`). **Physisch vs. wartend:** Zustandsblock unterscheidet jetzt klar „GESCHLOSSEN — Riegel zu (bestätigt)" von „WIRD GESCHLOSSEN/GEÖFFNET — wartet auf Box-Sync" (amber, Spinner). **WLAN:** „Von der Box gemeldet" nur noch für Netze, die der Server nicht ohnehin listet (Primär/Push rausgefiltert) → kein Doppel-„blackwater".
- **2026-07-12 (FW 0.2.32 — Log-Zeitstempel konsistent lokal, kein +2h-Knick)** — In den übertragenen Logs sprang der Zeitstempel mitten im Boot um +2 h (z. B. `[14:35:34] WiFi connected` → `[16:35:42] read Timeout` 11 s später). Ursache: Die Zeitzone (`Europe/Zurich`) wurde erst in `syncNtp()` gesetzt — also *nach* Boot-Banner und WiFi-Connect. Alle Zeilen davor stempelten in **UTC**, alle danach **lokal** (+2 h). Bei Wakes ohne WLAN (syncNtp nie erreicht) blieb alles UTC. Fix: TZ jetzt **ganz am Anfang von `setup()`** (`TZ_EUROPE_ZURICH` aus config.h), die redundante Stelle in `syncNtp` entfernt → alle Box-Log-Zeitstempel von der ersten Zeile an konsistent lokal.
- **2026-07-12 (FW 0.2.31 — Flash-Log-[E]-Spam richtig weg: stat() statt exists())** — Der 0.2.30-Fix war **wirkungslos**: `LittleFS.exists()` ist im Arduino-ESP32-Core selbst über ein internes read-`open()` implementiert und löst damit **genau dasselbe** `[E][vfs_api] open(): … does not exist` aus, das es vermeiden sollte — das Rauschen blieb bei jedem erfolgreichen Sync (kein Backlog → Datei fehlt). Jetzt Existenzprüfung über POSIX **`stat("/littlefs/backlog.log")`** (`backlogExists()`), das ENOENT still zurückmeldet, ohne das VFS-open-Logging. Rein kosmetisch (Sync war immer HTTP 200), aber hält Box- **und** hochgeladenes Server-Log sauber.
- **2026-07-12 (FW 0.2.30 — Flash-Log-Fix: kein [E]-vfs-Spam)** — Das Flash-Log (0.2.29) löste bei jedem erfolgreichen Sync harmlose, aber alarmierende ESP32-Kern-Fehlerzeilen aus: `FlashLog::toJson` öffnete `/backlog.log` read-only und `FlashLog::clear` löschte es — obwohl die Datei mangels Fehl-Sync gar nicht existiert → `[E][vfs_api.cpp] open(): … does not exist` bzw. `remove(): … does not exists`. Beide Zugriffe jetzt mit `LittleFS.exists(PATH)` vorgeschaltet → kein `[E]`-Rauschen mehr (Sync war ohnehin HTTP 200; rein kosmetisch, analog zum `nvs_open NOT_FOUND`-Fix). Kein Verhaltenswechsel.
- **2026-07-12 (FW 0.2.29 — persistentes Fehl-Sync-Log + konfigurierbares Sync-Intervall)** — Zwei gebündelte Features. **(1) Persistenter Flash-Log:** Bei einem fehlgeschlagenen Sync (kein WLAN/Server) gingen die Diagnosezeilen — insb. `WiFi: connect to '<ssid>' failed …` — im Deep-Sleep verloren und erreichten den Server nie. Neu: Modul `flash_log.h/.cpp` mountet die vorhandene, bislang ungenutzte **128-KB-`spiffs`-Partition** per **LittleFS** (ohne Partitionstabellen-Änderung → reiner Code-OTA). **Nur bei Fehl-Sync** wird der RAM-Log-Ausschnitt an `/backlog.log` angehängt (Rotation bei 16 KB), beim nächsten **erfolgreichen** Sync ungated hochgeladen und **erst nach `SyncResult::OK`** gelöscht (at-least-once, gleicher Seam wie Wake-Journal/OTA-Commit). Best-effort: schlägt der Mount fehl, sind alle Ops No-ops (blockiert nie Boot/Safety). Server (`box/sync`): neues `backlog`-Feld → Zeilen mit `⚠`-Marker in den bestehenden `DeviceLog` (ungated, gemeinsamer `createMany`/Prune) → sichtbar im `DeviceLogViewer`, auch ohne `logToServer`. **(2) Konfigurierbares Sync-Intervall:** Das dormante Heartbeat-Intervall (bisher hart 60 min) ist jetzt **pro Box über den Server** einstellbar (1–180 min, wie `offlineOpenHours`): `LockPolicy.syncIntervalMin` (Migration `add_sync_interval`), Feld im `DeviceSettingsForm`, in der Sync-Response geliefert, in NVS (`policy/syncInt`) gecacht, steuert den `goDeepSleep`-Timer. **Safety:** hart auf [1 min, 180 min] geklemmt; `MAX_SLEEP_S` von 3600 s auf **10800 s** (= Max-Intervall) angehoben, damit der monotone Offline-Failsafe-Zähler bei langem Intervall nicht unterzählt (Auto-Open feuert weiter rechtzeitig).
- **2026-07-08 (FW 0.2.28 — Aufwach-Journal)** — Jeder Box-Wake wird jetzt lückenlos im Geräte-Verlauf sichtbar, **auch offline**. Bisher lebte die Wake-Log-Zeile nur im RAM-Ring (`gLog`) → der Deep-Sleep löschte sie; ein Auto-Wake (`rtc_timer`), dessen Sync scheiterte, hinterließ keine Spur und wurde nie nachgereicht. Neu: Modul `wake_journal.h/.cpp` mit einem **deep-sleep-festen RTC-RAM-Ring** (64 Einträge à Grund/Zeit/Akku%; `RTC_DATA_ATTR` + Magic-Marker gegen Boot-Müll, kein zusätzlicher Sleep-Strom, kein Flash). `recordWake()` am Wake-Banner (`main.cpp`), Upload ungated im Sync-Body (`server_sync.cpp`), **Leeren nur bei `SyncResult::OK`** — dadurch ergibt sich der lückenlose Trail von selbst (fehlgeschlagene Wakes bleiben liegen, gehen beim nächsten geglückten Sync gebündelt hoch). Server (`box/sync`): `wakes[]` → je Wake ein `DeviceEvent{type:"WAKE"}` (Zeit-Klemme bei ungültiger RTC, Dedup über Timestamp gegen verlorene 200-Antworten, Retention-Cap 500/Gerät, Overflow-Notiz). Anzeige im bestehenden Verlauf: `EVENT_CONFIG.WAKE` „Aufwachen" (neutral), Detailseite lädt 200 statt 50 Events, `EventLog` bekommt Toggle „Routine-Wakes ausblenden". Keine Migration (`DeviceEvent.type` ist frei).
- **2026-07-08 (FW 0.2.27 — WLAN im Box-Log sichtbar)** — Das Box-Log zeigt jetzt bei **jedem** Sync das aktuell genutzte WLAN und **alle** Verbindungsversuche. Zwei Lücken geschlossen (`server_sync.cpp`): (1) im Wachfenster-Re-Sync ist WiFi schon verbunden → `connectWifi()` wurde übersprungen → gar kein WLAN-Eintrag; jetzt loggt `run()` in dem Fall `WiFi: still connected to '<ssid>' @ <ip> (<rssi> dBm)`. (2) Der Fast-Path-Reconnect (häufigster Wake-Fall) loggte nur den Erfolg — Versuch und Fehlschlag waren unsichtbar (stiller `disconnect()`); jetzt `WiFi: connecting (fast path) to '<ssid>' (ch N)` vorab, bei Erfolg mit RSSI, bei Fehlschlag `WiFi: fast path … failed → full scan`. Reine Log-Zeilen (fließen über `LOG*` in Ring-Puffer, `/dbg/log`, Server-Log, MQTT-Live), keine Verhaltensänderung am Connect-Pfad.
- **2026-07-08 (FW 0.2.16 — LED-Wake lückenlos + Logs nicht mehr zerrissen)** — **LED:** nach WiFi-Connect-Erfolg (`waitConnected`) sofort dauerhaft AN → keine „blinken → schwarz → an"-Lücke mehr während Sync/MQTT. Button-ISR-Sofort-Ack board-robust gemacht (`out_w1ts` bei active-HIGH statt fix `out_w1tc`). **Logs:** `collectSyncLogs()` kappt jetzt an der letzten `\n`-Grenze statt an fester Byte-Zahl → lange Zeilen (MQTT-Connect) zerreißen nicht mehr in zwei `DeviceLog`-Zeilen; unfertige Endzeile kommt komplett beim nächsten Aufruf.
- **2026-07-08 (FW 0.2.15 — WLAN-Sichtbarkeit + Remote-Vergessen)** — Der Server speichert jetzt die von der Box gemeldeten `knownSsids` (Namen, `Device.knownSsids` JSON, Migration `add_known_ssids`) und zeigt sie auf der Geräteseite unter „Von der Box gemeldet (NVS)". **Remote-Vergessen:** neues MQTT-Kommando `forget_wifi {ssid}` (Firmware `Command::FORGET_WIFI` → `NVS::deleteExtraNet` + Re-Sync); Dashboard-Button „vergessen" je Extra-Netz (Primär geschützt, kann nicht vergessen werden). Server nimmt das Netz sofort aus Anzeige- + Push-Liste, die Box bestätigt die reduzierte Liste beim nächsten Sync. Passwörter verlassen die Box weiterhin nie (nur Namen rauf, Push mit Passwort runter).
- **2026-07-08 (FW 0.2.14 — NVS-Namespaces vorab anlegen)** — `NVS::ensureNamespaces()` beim Boot legt die read-only gelesenen Namespaces (`state`/`policy`/`mqtt`/`nets`/`wifi`) einmal an. Vorher loggte `Preferences::begin(readOnly)` auf einer frischen Box — und dauerhaft für `nets` (bei jedem Sync über `knownSsids` gelesen, wenn keine Extra-WLANs gesetzt) — ein harmloses `nvs_open failed: NOT_FOUND` als ERROR. Neutraler `_ns`-Marker persistiert den leeren Namespace; die Loader prüfen weiter ihre echten Keys. Zweite Box (lötfrei via OTA-Übernahme geflasht) hat den Spam aufgedeckt.
- **2026-07-08 (FW 0.2.13 — Button-Sleep + debugMode raus + OTA-Hold sichtbar)** — **Taster-Sleep:** kurzer Tap = Sync (wie bisher), anhaltendes Halten ≥ `BTN_SLEEP_HOLD_MS` (1,5 s) → sofort Deep-Sleep („erst syncen, dann schlafen"), statt aufs 2-min-Fenster-Timeout zu warten. **debugMode komplett entfernt** (Firmware `gDebugMode` + Sync-Param + `!gDebugMode`-OTA-Gate; Server: Spalte gedroppt `drop_debug_mode`, Sync-/PATCH-Feld + Dashboard-Toggle raus) — die `/debug`-Seite war ohnehin unabhängig. **Auto-OTA:** gated jetzt nur noch auf *offen* (Brick-bei-ZU) + *Akku ≥ 40 %* (unbekannt/am Strom = ok); der Akku-Hold wird auf der Geräteseite **angezeigt** („vX verfügbar — Auto-OTA pausiert (Akku …%), manuell über /debug") statt still zu blockieren. Einziger expliziter Aus-Schalter bleibt „OTA einfrieren" (`otaDisabled`).
- **2026-07-08 (FW 0.2.12 — Debug-Reboot/Sleep + Hotspot-Abbrechen)** — Debug-Seite (`/debug`): neue Buttons **Reboot** (`/dbg/reboot` → `ESP.restart()`) und **Schlafen** (`/dbg/sleep` → regulärer `goDeepSleep()`), beide Antwort-vor-Aktion (Client-JS fängt den Abbruch). Setup-Hotspot: **„Setup verlassen"** (`/cancel`) → Reboot in den Normalbetrieb ohne Credentials anzufassen. Button erscheint NUR, wenn gültige Creds in NVS liegen (sonst gäbe es kein Ziel — die Box liefe direkt wieder in den Hotspot); Handler zusätzlich hart abgesichert. Deckt v. a. den versehentlichen 3-s-„WLAN-Wechsel"-Einstieg ab. Freeze-Invariante (Server/Token bei Verschluss) unberührt.
- **2026-07-08 (MQTT ist jetzt Standard, nicht abschaltbar)** — Das per-Box-Flag `mqttEnabled` (Rollout-Schalter, Default aus) fällt weg (Spalte gedroppt, Migration `drop_mqtt_enabled`, Dashboard-Toggle + PATCH-Feld raus). MQTT-Push wird jetzt fleet-weit ausgeliefert, sobald `MQTT_PUBLIC_HOST` gesetzt ist; ohne Broker-Host bleibt die Box heartbeat-only (abwärtskompatibel). Der schrittweise Rollout ist abgeschlossen (LMB2 stabil), daher Default statt Opt-in.
- **2026-07-08 (Fix: UNAUTHORIZED_OPEN bei Deadline-Drift)** — Der `timedLockExpired`-Fix (07-07) griff nicht: die Box öffnete real ~39 s VOR der Server-Deadline (RTC-Drift im Deep-Sleep, interner RC-Oszillator ~1 %/h) → `lockUntil` server-seitig noch knapp in der Zukunft → `boxLocked=true` → Fehlalarm blieb. Klassifikation jetzt mit **Drift-Toleranz** (`DEADLINE_DRIFT_MS = 10 min`) statt exaktem `<= now`: legitim, wenn keine Dauer-Sperre hält und die späteste timed Deadline ≤ `now + 10 min` liegt. Tamper (deutlich vor Deadline / Simple-Lock / ohne timed Sperre) bleibt `UNAUTHORIZED_OPEN`. Sicherheitskosten vernachlässigbar (nur die letzten ~10 min vor einer echten Deadline).
- **2026-07-08 (FW 0.2.11 — unexp-Baseline-Reset)** — Der `unexp`-Zähler (Brownout/Power-on, NVS `diag`) wird einmalig fleet-weit auf 0 gestellt: bisheriger Stand (LMB2: 31) war historisch von Bench-/USB-Power-ons aufgebläht, im Prod-Log 0 echte Feld-Brownouts. Marker-gesichert (`statbase`-Key = `STAT_BASELINE`) in `recordBoot()` → nullt genau einmal beim ersten Boot nach dem OTA, kein Kommando/Trigger nötig; erneutes Nullen durch Hochzählen von `STAT_BASELINE`. Ab hier zählt `unexp` nur noch echte Feld-Brownouts/Power-ons — sauberer Baseline für die Brownout-Beobachtung.
- **2026-07-28 (FW 0.2.36 — Vorwarnung vor der Funkstille-Notöffnung + Datum im Log)** — Eine Box konnte einen ganzen Tag lang jeden stündlichen Sync verfehlen, ohne dass irgendwo eine Warnung stand; sichtbar wurde der Zustand erst, als nach `offlineOpenHours` der Offline-Failsafe zuschlug und der Riegel autonom aufging — für den Träger überraschend, für die Keyholderin unbemerkt (Issue #1). Neu zeigen die Geräte-Karte **und** der Tracker (Box-Karte des Subs wie Keyholder-Sicht, dazu das MCP-Feld `failsafeWarnings`), seit wann die Box ohne Kontakt ist und wann die Not-Öffnung fällig wäre — dezent ab der Hälfte des Fensters, in Warnfarbe ab drei Vierteln. Der Zähler entsteht **serverseitig aus der vergangenen Zeit**, nie aus einem Box-Feld: eine Box ohne Netz kann ihre eigene Funkstille nicht melden. Am Failsafe selbst ändert sich nichts — er ist richtig so, es fehlte nur die Vorwarnung. Mitgenommen: die Akku-Notöffnung wird ebenso angekündigt (Schwelle wandert dafür an den Tracker), und die Low-Batt-Zeile schweigt nicht mehr am Ladekabel — `isLowBattery` fragt den Ladezustand gar nicht und latcht bis `BATT_RECOVER_PCT`. **Log:** die Zeitstempel tragen jetzt das Datum (`[28.07. 12:00:00]`) — ein Backlog aus tagelanger Funkstille liess sich vorher keinem Tag mehr zuordnen. Kostet ~7 Bytes je Zeile im 6-KB-RAM-Ring (~10 % weniger Zeilen), bewusst in Kauf genommen: ein Tageswechsel-Marker fiele genau im langen Offline-Fall aus dem Ring.
